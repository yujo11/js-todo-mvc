...
const TodoView = class {
  ...
  renderTodos(todos) {
    this.$todoList.innerHTML = todos
      .map((todo, index) => {
        return todo.editing
          ? this.#editTemplate(todo, index)
          : this.#todoTemplate(todo, index);
      })
      .join("");
  }
  ...
  #editTemplate(todo, index) {
    return `<li data-index="${index}" class="editing">
              <div class="view">
  ...
  #todoTemplate(todo, index) {
    return `<li data-index="${index}" class="todo-item
